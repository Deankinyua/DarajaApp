defmodule ExampleWeb.Gen.Live do
  @moduledoc false

  def generate_from_cli(argv) do
    {api, resource, opts, _rest} = ExampleWeb.Gen.parse_opts(argv)
  ...
  def generate(api, resource, opts \\ []) do
    ...
    if !Spark.Dsl.is?(api, Ash.Domain) do
    ...
    web_live = Path.join([web_path(), "#{assigns[:resource_singular]}"])
    ...
    write_formatted_template(
      "example_web.gen.live/index.ex.eex",
      "pages/index.#{assigns[:resource_singular]}.ex",
      web_live,
      assigns,
      generate_opts
    )
    ...
    write_formatted_template(
      "example_web.gen.live/list.ex.eex",
      "components/list.#{assigns[:resource_singular]}.ex",
      web_live,
      assigns,
      generate_opts
    )
    ...
    write_formatted_template(
      "example_web.gen.live/list.card.ex.eex",
      "components/list.card.#{assigns[:resource_singular]}.ex",
      web_live,
      assigns,
      generate_opts
    )
    ...
    write_formatted_template(
      "example_web.gen.live/table.ex.eex",
      "components/table.#{assigns[:resource_singular]}.ex",
      web_live,
      assigns,
      generate_opts
    )
    ...
    write_formatted_template(
      "example_web.gen.live/table.row.ex.eex",
      "components/table.row.#{assigns[:resource_singular]}.ex",
      web_live,
      assigns,
      generate_opts
    )
    ...
    if assigns[:update_action] || assigns[:create_action] do
      write_formatted_template(
        "example_web.gen.live/form_component.ex.eex",
        "components/form.#{assigns[:resource_singular]}.ex",
        web_live,
        assigns,
        generate_opts
      )
    ...
    write_formatted_template(
      "example_web.gen.live/show.ex.eex",
      "pages/show.#{assigns[:resource_singular]}.ex",
      web_live,
      assigns,
      generate_opts
    )
  ...
  defp add_resource_assigns(assigns, api, resource, opts) do
    short_name =
      resource
    ...
    get_by_pkey = get_by_pkey(api, resource, pkey, opts)
    ...
    Keyword.merge(assigns,
      ...
      destroy: destroy(api, short_name, get_by_pkey, resource, opts),
      pkey: pkey,
  ...
  defp destroy(api, short_name, get_by_pkey, resource, opts) do
  ...
  defp template(path) do
    :code.priv_dir(:example_web) |> Path.join("templates") |> Path.join(path)
  end
          ...
          Ash.Type.Integer ->
            ~s(
              <Layout.col class="space-y-1.5">
                <label for={@form[#{inspect(field.name)}].name <> "_field"}>
                ...
                <Input.text_input
                  id={@form[#{inspect(field.name)}].name}
                  name={@form[#{inspect(field.name)}].name}
                  placeholder="Enter value"
                  type="number"
                  field={@form[#{inspect(field.name)}]}
                  value={@form[#{inspect(field.name)}].value}
                  error={@form[#{inspect(field.name)}].errors != [] && #{inspect("field_touched(@form[#{inspect(field.name)}], :name) == true")}}
                  error_message={#{"get_field_errors(@form[#{inspect(field.name)}], :name)"}}
                  required="true"
                />
              </Layout.col>
            )
            ...
            ~s(
              <Layout.col class="space-y-1.5">
                ...
                <Input.text_input
                  ...
                  field={@form[#{inspect(field.name)}]}
                  value={@form[#{inspect(field.name)}].value}
                  error={@form[#{inspect(field.name)}].errors != [] && #{"field_touched(@form[#{inspect(field.name)}], :name) == true"}}
                  error_message={#{"get_field_errors(@form[#{inspect(field.name)}], :name)"}}
                  required="true"
                  step="any"
                />
                ...
                ~s"""
                  <Layout.col class="space-y-1.5">
                    ...
                    >
                      ...
                      <:item :for={value <-  Ash.Resource.Info.attribute(#{inspect(resource)}, #{inspect(field.name)}).constraints[:one_of]}>
                        <%= value %>
                      </:item>
